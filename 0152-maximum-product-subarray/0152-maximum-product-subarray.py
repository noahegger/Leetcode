...
class Solution:
    def maxProduct(self, nums: List[int]) -> int:
        ...
        for num in nums[1:]:
            # Store the current max_thus in a temporary variable
            print("max_thus:", max_thus)
            print("min_thus:", min_thus)
            print("result:", result)
            temp_max = max_thus
            # Calculate the new max_thus and min_thus
            max_thus = max(num, num * max_thus, num * min_thus)
            min_thus = min(num, num * temp_max, num * min_thus)

            print("new_max", max_thus)
            print("new_min", min_thus)
            
            # Update the result with the maximum value of result and max_thus
